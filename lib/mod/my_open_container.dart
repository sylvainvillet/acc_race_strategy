...
/// Signature for `action` callback function provided to [MyOpenContainer.openBuilder].
///
/// Parameter `returnValue` is the value which will be provided to [MyOpenContainer.onClosed]
...
/// [MyOpenContainer].
///
/// The `action` callback provided to [MyOpenContainer.openBuilder] can be used
...
/// [MyOpenContainer].
///
/// The `action` callback provided to [MyOpenContainer.closedBuilder] can be used
...
/// The [MyOpenContainer] widget's fade transition type.
///
...
/// Callback function which is called when the [MyOpenContainer]
/// is closed.
...
@optionalTypeArgs
class MyOpenContainer<T extends Object> extends StatefulWidget {
  /// Creates an [MyOpenContainer].
  ///
  ...
  @override
  _MyOpenContainerState<T> createState() => _MyOpenContainerState<T>();
}

class _MyOpenContainerState<T> extends State<MyOpenContainer<T>> {
...
class _OpenContainerRoute<T> extends ModalRoute<T> {
  ...
  static final TweenSequence<Color> _scrimFadeInTween = TweenSequence<Color>(
    <TweenSequenceItem<Color>>[
      ...
      TweenSequenceItem<Color>(
        tween: ConstantTween<Color>(Colors.white),
        weight: 4 / 5,
      ),
    ],
  );
